...
from research_edm.DATA.class_mapping import map_category
...
from research_edm.inference.model_instantiation import instantiate_default_dryrun, parse_ctor_params, cls_task, \
    ModelWrapper, get_data_type, categories_type
...
def cross_train_model(wrapped_model, features, labels, test_size):
    ready_for_eval = []
...
def infer_dataset(active_models, models_configs, dset, transform, norm_flag):
    ...
    if active_models is None or len(active_models) == 0:
        ...
        wrapped_models = parse_ctor_params(models_configs, active_models)
    ...
    for wrapped_model in wrapped_models:
        ...
        if wrapped_model.task_type == cls_task:
            ready_for_eval = cross_train_model(wrapped_model, features, one_hot_labels, test_size=0.1)
        else:  # multi-collinearity problems for one-hot regression
            ready_for_eval = cross_train_model(wrapped_model, features, labels, test_size=0.1)
